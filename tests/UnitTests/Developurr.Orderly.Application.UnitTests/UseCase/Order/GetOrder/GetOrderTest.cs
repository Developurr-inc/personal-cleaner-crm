...
namespace Developurr.Orderly.Application.UnitTests.UseCase.Order.GetOrder;
...
public sealed class GetOrderTest
{
    ...
    public async void GivenValidInput_WhenCallingExecute_ThenShouldCallGetByIdAsync()
    {
        ...
        orderRepositoryMock.Setup(x => x.GetByIdAsync(input.OrderId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(order);
