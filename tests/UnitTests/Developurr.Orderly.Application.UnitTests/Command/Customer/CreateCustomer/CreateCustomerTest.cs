...
namespace Developurr.Orderly.Application.UnitTests.Command.Customer.CreateCustomer;
...
public class CreateCustomerTest
{
    ...
    public async void GivenValidInput_WhenCallingExecute_ThenShouldCallInsertAsyncOnce()
    {
        ...
        customerRepositoryMock.Verify(
            x => x.InsertAsync(It.IsAny<Domain.Customer.Customer>(), CancellationToken.None),
            Times.Once
        );
    }
    ...
    public async void GivenInvalidInput_WhenCallingExecute_ThenShouldReturnDomainValidationExceptionWithMessage()
    {
        ...
        var exception = await Record.ExceptionAsync(
            () => createCustomerUseCase.Handle(input, CancellationToken.None)
        );
