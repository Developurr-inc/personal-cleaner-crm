...
public static partial class ValidationRules
{
    ...
    public static void ValidateStringLength(string value, string fieldName, int minLength, int maxLength, IValidator errors)
    {
        if (value.Length < minLength || value.Length > maxLength)
        {
            errors.AddValidationError($"'{fieldName}' should be between {minLength} and {maxLength} characters.");
        }
    }


    public static void ValidateMaxStringLength(string value, string fieldName, int maxLength, IValidator errors)
    {
    ...
    public static void ValidatePhoneNumber(string phone, string fieldName, IValidator errors)
    {
        ...
        Regex validatePhoneNumberRegex = new Regex(@"^(\+55\s?)?(\(\d{2}\)\s?)?(\d{4,5}-\d{4}|\d{11})$");
